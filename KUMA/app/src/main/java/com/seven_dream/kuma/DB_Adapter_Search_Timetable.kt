...
class DB_Adapter_Search_Timetable(mContext: Context) {
    ...
    //lecture_nameを指定してlectureIDを取得
    fun getLecture_idByName(lecture_name: Editable, num:Int): Int{
    ...
    fun getLecture_idByQuarter(quarter: Int, num: Int): Int{
        val selectSql : String = "select lecture_id from lecture where quarter = ?"
        val cursor: Cursor = db.rawQuery(selectSql, arrayOf(quarter))
        //Log.d("opal",cursor.toString())
        var id : Int = 0//最終的に表示
        try{
            if(cursor.moveToNext()) {
